...
class Usuarios
{
    ...
    public static function mostrarInfo($token)
    {
        ...
        try {
            $sql = "SELECT a.*, FLOOR(DATEDIFF(NOW(),a.fecha_nacimiento)/365) AS edad , ifnull((select count(*) from paciente where usuario_asignado=u.email),0) AS pacientes_asignados from profesor a, usuario u where u.token ='$token' and u.email = a.id_usuario";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $data = $stmt->fetchAll(PDO::FETCH_ASSOC);

            if (empty($data)) {
                $sql = "SELECT a.*, g.nombre as nombre_grupo ,FLOOR(DATEDIFF(NOW(),a.fecha_nacimiento)/365) AS  edad , ifnull((select count(*) from paciente where usuario_asignado=u.email),0) AS pacientes_asignados from tecnico a, usuario u, grupo g where g.id = a.id_grupo and u.token ='$token' and u.email = a.id_usuario";
                $stmt = $pdo->prepare($sql);
                $stmt->execute();
                $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
                if(empty($data)){
                    $sql = "SELECT a.* , FLOOR(DATEDIFF(NOW(),a.fecha_nacimiento)/365) AS edad , ifnull((select count(*) from paciente where usuario_asignado=u.email),0) AS pacientes_asignados from tecnico a, usuario u where  u.token ='$token' and u.email = a.id_usuario";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
                    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
                }
